...
st.markdown("""
    ...
    <style>
        ...
        /* Neon Linear/Vercel Button Customization — excludes file uploader & camera buttons */
        .stButton > button,
        ...
        /* --- FILE UPLOADER: full dark redesign --- */
        ...
        [data-testid="stFileUploader"] section {
            ...
            border: 1px dashed rgba(79,158,255,0.45) !important;
        ...
        [data-testid="stFileUploader"] section > input + div > span {
            color: #64748b !important;
            font-size: 12px !important;
        }
        [data-testid="stFileUploader"] section button {
            background: rgba(30, 40, 80, 0.9) !important;
            border: 1px solid rgba(79,158,255,0.5) !important;
            ...
            box-shadow: 0 0 8px rgba(79,158,255,0.15) !important;
            /* CRITICAL: reset gradient from global rule */
            background-image: none !important;
        }
        [data-testid="stFileUploader"] section button:hover {
            background: rgba(79,158,255,0.2) !important;
            border-color: rgba(79,158,255,0.9) !important;
            color: #ffffff !important;
            box-shadow: 0 0 12px rgba(79,158,255,0.35) !important;
            transform: none !important;
        }
        /* --- CAMERA INPUT: full dark redesign --- */
        ...
        /* ALL buttons inside camera input (Take Photo, Retake) */
...
def save_data():
    ...
    MERGE-FIRST STRATEGY: Always read what the AI engine has independently
    written to disk before saving, so we never erase AI-logged attendance.
    The UI session_state is authoritative for student/timetable metadata,
    but AI attendance records written by the engine take precedence for
    any student not already present in session_state for that period slot.
    """
    try:
        # ── Step 1: Read current on-disk period_attendance (AI may have written it) ──
        existing_period_attendance = []
        if os.path.exists(DATA_FILE):
            try:
                with open(DATA_FILE, 'r') as _rf:
                    _existing = json.load(_rf)
                existing_period_attendance = _existing.get("period_attendance", [])
            except Exception:
                existing_period_attendance = []

        # ── Step 2: Build lookup dicts keyed by (date, period, subject) ──
        disk_index = {}
        for rec in existing_period_attendance:
            key = (rec.get("date"), rec.get("period"), rec.get("subject"))
            disk_index[key] = rec

        session_index = {}
        for rec in st.session_state.period_attendance:
            key = (rec.get("date"), rec.get("period"), rec.get("subject"))
            session_index[key] = rec

        # ── Step 3: Merge — session has priority for metadata; disk fills in
        # any student statuses the session doesn't know about (AI-written) ──
        merged_attendance = []
        all_keys = set(disk_index.keys()) | set(session_index.keys())
        for key in all_keys:
            if key in session_index:
                sess_rec = session_index[key]
                if key in disk_index:
                    disk_att = disk_index[key].get("attendance", {})
                    sess_att = sess_rec.setdefault("attendance", {})
                    for roll, status in disk_att.items():
                        # Only fill in if session doesn't already have a status,
                        # or if the disk status is a "better" status (e.g. Exited)
                        if roll not in sess_att:
                            sess_att[roll] = status
                        elif status in ["Exited", "Present (Excused)", "Auto-Carried"] and sess_att[roll] in ["Absent"]:
                            sess_att[roll] = status
                merged_attendance.append(sess_rec)
            else:
                # Only on disk (pure AI record) — keep as-is
                merged_attendance.append(disk_index[key])

        # ── Step 4: Update session_state so UI reflects merged state ──
        st.session_state.period_attendance = merged_attendance

        data_to_save = {
            "students": [],
            "period_attendance": merged_attendance,
        ...
        # Write to JSON file with retries to avoid race condition
        for _ in range(5):
            try:
...
def delete_student_assets(roll_no):
    """Clean up student dataset folders, embeddings, and reload recognition engine."""
    import os
    import shutil
    import pickle
    ...
    if os.path.exists(dataset_path):
        try:
            for folder in os.listdir(dataset_path):
                if folder.startswith(f"{roll_no}_") or folder == str(roll_no):
                    shutil.rmtree(os.path.join(dataset_path, folder))
        except Exception as e:
            pass
            
    # 2. Clean face_embeddings.pkl
    pkl_path = "data/face_embeddings.pkl"
    if os.path.exists(pkl_path):
        try:
            with open(pkl_path, 'rb') as f:
                emb = pickle.load(f)
            if str(roll_no) in emb:
                del emb[str(roll_no)]
    ...
    # 3. Regenerate face embeddings to make sure face_embeddings.pkl is 100% in sync
    try:
        from vision.generate_embeddings import generate_face_embeddings
        generate_face_embeddings(verbose=False)
    except Exception as e:
        pass
        
    # 4. Hot-reload embeddings in live recognition engine if running
    try:
        existing_engine = st.session_state.get('recognition_engine')
        if existing_engine is not None and st.session_state.get('recognition_running', False):
            existing_engine.reload_embeddings()
    except Exception as e:
        pass
...
def check_overlapping_periods(day, new_start, new_end):
    ...
    try:
        ns = parse_time(new_start)
        ne = parse_time(new_end)
    except:
        ns, ne = new_start.strip(), new_end.strip()

    for period in periods:
        es = parse_time(period.get("start_time", ""))
        ee = parse_time(period.get("end_time", ""))
        
        try:
            if not (ne <= es or ns >= ee):
                return True
        except TypeError:
            # Fallback to string comparison if format was weird
            if not (str(ne) <= str(es) or str(ns) >= str(ee)):
                return True
...
def show_dashboard():
    ...
    # 1.5 System Rules & Instructions
    st.markdown("""
        <div class="animated-card" style="border-left: 4px solid #3b82f6 !important; margin-bottom: 24px;">
            <h3 style="margin-top:0; color:#3b82f6; font-size:18px;">⚖️ System Rules & Instructions</h3>
            <div style="display:flex; gap:20px; flex-wrap:wrap;">
                <div style="flex:1; min-width:300px;">
                    <h4 style="color:#94a3b8; font-size:14px; margin-bottom:8px;">👨‍🏫 FOR TEACHERS</h4>
                    <ul style="color:#cbd5e1; font-size:13px; line-height:1.6; padding-left:20px; margin:0;">
                        <li><b>Start Camera on Time:</b> Ensure the "Real-Time Face Recognition" is running before the period begins.</li>
                        <li><b>Verify Auto-Carries:</b> If a student stays for a consecutive period, verify their presence in the Live Camera tab to mark them "Present".</li>
                        <li><b>Confirm Exits:</b> If a student leaves mid-class, confirm their exit in the UI to log their Exit Time.</li>
                        <li><b>Gatepass Approvals:</b> Manage digital gatepasses for late students so they are marked "Excused" instead of "Late".</li>
                    </ul>
                </div>
                <div style="flex:1; min-width:300px;">
                    <h4 style="color:#94a3b8; font-size:14px; margin-bottom:8px;">🎓 FOR STUDENTS</h4>
                    <ul style="color:#cbd5e1; font-size:13px; line-height:1.6; padding-left:20px; margin:0;">
                        <li><b>Dwell Time (1.5s):</b> You must look directly at the camera for at least 1.5 seconds to register your attendance.</li>
                        <li><b>Late Grace Period:</b> Arriving more than 10 minutes after the scheduled start time will mark you as "Late" (unless excused).</li>
                        <li><b>Continuous Presence:</b> Exiting the room for more than 15 seconds will trigger an Exit confirmation. Do not leave the room unexcused.</li>
                        <li><b>Smart Auto-Carry:</b> If you stay for consecutive periods, the camera will automatically carry your attendance over.</li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    if not today_periods:
        st.info(f" No classes scheduled for {search_day_name}, {search_date_str}. Enjoy your day off!")
    else:
    ...
    if not st.session_state.students:
        st.warning(" No students registered in the college database yet. Go to 'Manage Students' to register students.")
    else:
...
def show_manage_students():
    ...
    with tab1:
        if len(st.session_state.students) == 0:
            ...
            with btn_col:
                if st.button(" Remove Student", use_container_width=True, key="dir_remove_btn_merged"):
                    ...
                    delete_student_assets(removed_roll)
                    save_data()
    ...
    with tab2:
        ...
        with st.form("add_student_form_merged"):
            ...
            with col4:
                branch = st.text_input("Branch*", value=tt_dept)
                
            student_class = st.text_input("Class*", value=tt_cls)
                
            img_col1, img_col2 = st.columns(2)
            with img_col1:
                uploaded_file = st.file_uploader("Upload Image File", type=["jpg", "jpeg", "png"], label_visibility="visible")
            with img_col2:
                camera_photo = st.camera_input("Take Camera Photo", label_visibility="visible")
                        ...
                        if len(faces) == 0:
                            ...
                            if match and match['similarity'] > 0.45:
                                ...
                                if result.get('status') in ['success', 'partial_success']:
                                    ...
                                    if st.session_state.recognition_engine is not None:
                                        st.session_state.recognition_engine.reload_embeddings()
                                else:
                                    st.error(f" Warning: Failed to generate embeddings: {result.get('error', 'Unknown Error')}")
...
def show_student_directory():
    ...
    with col3:
        if st.button(" Remove Student", use_container_width=True, key="dir_remove_btn"):
...
def show_timetable():
    ...
    with col1:
        if st.button("Reset Day Timetable", key="btn_reset_day"):
            st.warning("Select the day to reset in the form below, then click this button again.")
    
    with col2:
        if st.button("Reset Full Timetable", key="btn_reset_all"):
            if st.session_state.get("confirm_reset_all"):
                ...
                st.error("Click the 'Confirm Reset' button below to reset the full timetable.")
    ...
    st.subheader("Class Schedules View")
    ...
    # --- Timetable Settings for HTML Render ---
    if "tt_settings" not in st.session_state:
        st.session_state.tt_settings = {
            "department": "DEPARTMENT OF CSE(AI&ML)",
            "class": "TY",
            "term": "II",
            "wef_date": "28/04/2026",
            "hall": "224",
            "batch": "(DBATU Non-Autonomous Batch)"
        }
        
    st.markdown("#### Customize Timetable Header")
    with st.container(border=True):
        st.write("Edit the details shown at the top of the printable timetable.")
        s_col1, s_col2 = st.columns(2)
        with s_col1:
            st.session_state.tt_settings["department"] = st.text_input("Department", st.session_state.tt_settings["department"])
            st.session_state.tt_settings["class"] = st.text_input("Class", st.session_state.tt_settings["class"])
            st.session_state.tt_settings["batch"] = st.text_input("Batch Info", st.session_state.tt_settings["batch"])
        with s_col2:
            st.session_state.tt_settings["term"] = st.text_input("Term", st.session_state.tt_settings["term"])
            st.session_state.tt_settings["wef_date"] = st.text_input("W.E.F Date", st.session_state.tt_settings["wef_date"])
            st.session_state.tt_settings["hall"] = st.text_input("Hall No", st.session_state.tt_settings["hall"])
            
    # --- HTML TIMETABLE RENDER ---
    s = st.session_state.tt_settings
    
    table_html = f"""
    <style>
        .tt-container {{ width: 100%; overflow-x: auto; background: transparent; color: #f8fafc; font-family: 'Inter', sans-serif; border: 1px solid rgba(255,255,255,0.2); padding: 0; margin-bottom: 20px; border-radius: 8px; }}
        .tt-header {{ text-align: center; margin-bottom: 0px; padding-top: 15px; }}
        .tt-header h3 {{ font-size: 16px; margin: 0; font-weight: 800; color: #e2e8f0; text-transform: uppercase; letter-spacing: 0.05em; }}
        .tt-header h2 {{ font-size: 18px; margin: 6px 0; font-weight: 900; color: #ffffff; }}
        .tt-header h4 {{ font-size: 16px; margin: 0 0 10px 0; font-weight: 700; color: #a78bfa; }}
        .tt-info {{ display: flex; justify-content: space-between; font-size: 13px; font-weight: 700; border-top: 1px solid rgba(255,255,255,0.2); border-bottom: 1px solid rgba(255,255,255,0.2); padding: 8px 15px; background: rgba(255,255,255,0.03); color: #cbd5e1; }}
        .tt-batch {{ text-align: center; font-size: 12px; font-weight: 700; margin: 0; border-bottom: 1px solid rgba(255,255,255,0.2); padding: 5px; background: rgba(255,255,255,0.02); color: #94a3b8; text-transform: uppercase; letter-spacing: 0.05em; }}
        table.tt-table {{ width: 100%; border-collapse: collapse; text-align: center; font-size: 13px; background: transparent; }}
        table.tt-table th, table.tt-table td {{ border: 1px solid rgba(255,255,255,0.15); padding: 8px; color: #e2e8f0; vertical-align: middle; }}
        table.tt-table th {{ font-weight: 800; background-color: rgba(255,255,255,0.05); color: #ffffff; }}
        .tt-recess {{ text-orientation: upright; writing-mode: vertical-rl; font-weight: 900; letter-spacing: 6px; font-size: 16px; color: #fb923c; background-color: rgba(251,146,60,0.05) !important; }}
    </style>
    <div class="tt-container">
        <div class="tt-header">
            <h3>M. S. P. MANDAL's</h3>
            <h2>DEOGIRI INSTITUTE OF ENGINEERING AND MANAGEMENT STUDIES, CHHATRAPATI SAMBHAJIANAGAR</h2>
            <h4>{s["department"]}</h4>
        </div>
        <div class="tt-info">
            <span>Class: {s["class"]}</span>
            <span>TIME TABLE FOR THE YEAR 2025-26</span>
            <span>TERM- {s["term"]} &nbsp;&nbsp;&nbsp;&nbsp; W.E.F {s["wef_date"]}</span>
            <span>HALL NO: - {s["hall"]}</span>
        </div>
        <div class="tt-batch">{s["batch"]}</div>
        <table class="tt-table">
            <tr>
                <th>PERIODS</th>
                <th>I</th>
                <th>II</th>
                <th rowspan="8" class="tt-recess">RECESS</th>
                <th>III</th>
                <th>IV</th>
                <th rowspan="8" class="tt-recess">RECESS</th>
                <th>V</th>
            ...
            <tr>
                <th>TIMING</th>
                <th>10.15 a.m. to<br>11.15 a.m.</th>
                <th>11.15 a.m. to<br>12.15 p.m.</th>
                <th>01.15 p.m. to<br>02.15 p.m.</th>
                <th>02.15 p.m. to<br>03.15 p.m.</th>
                <th>03.30 p.m. to<br>04.30 p.m.</th>
                <th>04.30 p.m. to<br>05.30 p.m.</th>
            </tr>
    """
    
    # Calculate dates from the W.E.F date for displaying with weekdays
    try:
        base_date = datetime.strptime(s["wef_date"], "%d/%m/%Y")
    except ValueError:
    ...
    for i, day in enumerate(["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]):
        ...
        periods = st.session_state.timetable.get(day, [])
        lecture_periods = [p for p in periods if p.get('type') not in ['recess', 'break']]
        
        cols_html = ""
        for j in range(6):
            if j < len(lecture_periods):
                p = lecture_periods[j]
                subj = p.get('subject', '-')
                p_name = p.get('period', '')
                # Replicate the UI formatting (Subject on top, Teacher/Lab below)
                cols_html += f"<td>{subj}<br><small>({p_name})</small></td>"
            else:
                cols_html += "<td>--</td>"
                
        table_html += f"""
            <tr>
                <th>{day.upper()}<br>({date_str})</th>
                {cols_html}
            </tr>
        """
    ...
    # Strip newlines to prevent Streamlit from wrapping HTML tags in <p> and breaking the table
    ...
    def generate_excel_timetable():
        ...
        recess_fill = PatternFill(start_color="FFF2CC", end_color="FFF2CC", fill_type="solid")
        
        # Institution Headers
        ws.merge_cells("A1:I1")
        ws["A1"] = "M. S. P. MANDAL's"
        ws["A1"].font = Font(bold=True, size=14)
        ws["A1"].alignment = center_align
        
        ws.merge_cells("A2:I2")
        ws["A2"] = "DEOGIRI INSTITUTE OF ENGINEERING AND MANAGEMENT STUDIES, CHHATRAPATI SAMBHAJIANAGAR"
        ...
        ws["A4"] = f"Class: {s['class']}"
        ws.merge_cells("D4:F4")
        ws["D4"] = "TIME TABLE FOR THE YEAR 2025-26"
        ws.merge_cells("G4:I4")
        ws["G4"] = f"TERM- {s['term']}    W.E.F {s['wef_date']}    HALL: {s['hall']}"
        ...
        ws["A5"] = s["batch"]
        ws["A5"].font = bold_center
        ws["A5"].alignment = center_align
        
        # Table Headers
        headers = ["PERIODS", "I", "II", "RECESS", "III", "IV", "RECESS", "V", "VI"]
        ws.append(headers)
        
        timings = ["TIMING", "10.15 a.m. to\n11.15 a.m.", "11.15 a.m. to\n12.15 p.m.", "", 
                   "01.15 p.m. to\n02.15 p.m.", "02.15 p.m. to\n03.15 p.m.", "", 
                   "03.30 p.m. to\n04.30 p.m.", "04.30 p.m. to\n05.30 p.m."]
        ws.append(timings)
        
        # Merge RECESS columns vertically across all days (row 6 to 13)
        ws.merge_cells("D6:D13")
        ws["D6"] = "R E C E S S"
        ws["D6"].font = bold_center
        ws["D6"].alignment = center_align
        ws["D6"].fill = recess_fill
        
        ws.merge_cells("G6:G13")
        ws["G6"] = "R E C E S S"
        ws["G6"].font = bold_center
        ws["G6"].alignment = center_align
        ws["G6"].fill = recess_fill
        
        # Apply borders and backgrounds to headers
        ...
        for i, day in enumerate(["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]):
            ...
            for col_idx in range(1, 9):
                if col_idx == 3 or col_idx == 6:  # RECESS columns
                    row_data.append("")
                    continue
                if p_idx < len(lecture_periods):
                    p = lecture_periods[p_idx]
                    row_data.append(f"{p.get('subject', '-')}\n({p.get('period', '')})")
                    p_idx += 1
                else:
                    row_data.append("--")
            ws.append(row_data)
    ...
    st.download_button(
        label=" Export Timetable to Excel",
        data=generate_excel_timetable(),
        file_name=f"{s['class']}_{s['department'].replace(' ', '_')}_Timetable.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        use_container_width=True
    )
    
    # Keep the editable cards below so user can delete/modify
    st.markdown("#### Edit/Delete Specific Periods")
    with st.container(border=True):
        st.markdown("### Weekly Days Details")
        cols = st.columns(3)
            ...
            with cols[idx % 3]:
                ...
                if not periods:
                    ...
                    for p_idx, p in enumerate(periods):
                        ...
                        if st.button("Delete Period", key=f"del_{day}_{p_idx}", use_container_width=True):
...
def show_live_attendance():
    ...
    if len(st.session_state.students) == 0:
        st.warning(" No students in the system. Add students first!")
        return
    ...
    with col2:
        ...
        if absent_list:
            for roll_no, status in absent_list:
                student = get_student_by_roll_no(roll_no)
                student_name = student.get("name", roll_no) if student else roll_no
                status_emoji = "" if status == "Absent" else "⏱"
...
def show_reports_page():
    ...
    with tab1:
        ...
        if not timetable:
            ...
            for i, p in enumerate(timetable):
                ...
                valid_statuses = ["Present", "Present (Excused)", "Late", "Auto-Carried", "Exited", "Exited (Unverified)"]
                present_count = sum(1 for v in per_records.values() if v.get("status") in valid_statuses)
            ...
            for idx, student in enumerate(st.session_state.students, 1):
                ...
                for p in timetable:
                    ...
                    if rec and rec.get("status") in ["Present", "Present (Excused)", "Late", "Auto-Carried", "Exited", "Exited (Unverified)"]:
                        if rec.get("status") == "Late":
                            row[col_name] = "⏱"
                        elif rec.get("status") == "Present (Excused)":
                            row[col_name] = "E"
                        else:
                            row[col_name] = ""
            ...
            for _, row in df[display_cols].iterrows():
                ...
                for c in period_cols_hdr:
                    ...
                    if v == "":
                        ...
                        badge = '<span style="background:rgba(56,189,248,0.15);color:#38bdf8;border:1px solid rgba(56,189,248,0.3);border-radius:6px;padding:3px 10px;font-size:11px;font-weight:700;"> Excused</span>'
                    else:
                        ...
                        with drill_cols[i % len(drill_cols)]:
                            st.markdown(f"""
                                <div class="animated-card" style="padding:14px; text-align:center; border-left:4px solid {accent} !important;">
                                    ...
                                    <div style="font-size:22px; font-weight:900; color:{accent}; margin:8px 0;">{icon} {status_text}</div>
                                    <div style="font-size:11px; color:#64748b;">{"Entry: " + t_val if is_present else "No Record"}</div>
                                </div>
                            """, unsafe_allow_html=True)
    ...
    with tab2:
            ...
            <div style="background:linear-gradient(135deg,rgba(79,158,255,0.06),rgba(167,139,250,0.06));border:1px solid rgba(79,158,255,0.15);border-radius:16px;padding:20px;margin-bottom:20px;">
                ...
                <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:8px;">
                    ...
                    <div style="background:rgba(52,211,153,0.08);border:1px solid rgba(52,211,153,0.2);border-radius:10px;padding:10px;text-align:center;">
                        <div style="font-size:16px;"></div><div style="font-size:11px;font-weight:700;color:#34d399;margin-top:4px;">Week-wise</div>
        ...
        scope_tabs = st.tabs([" Day-wise", " Date-wise", " Period-wise", " Week-wise", " Month-wise"])
        ...
        with scope_tabs[3]:
            st.markdown("""<div class="animated-card" style="padding:18px;margin-bottom:14px;">
                <div style="font-size:14px;font-weight:700;color:#34d399;margin-bottom:4px;"> Week-wise Report</div>
...
def _generate_and_show_report(scope_type: str, scope_value: str, selected_period_id, live_tt: dict):
    ...
    if not period_slots:
        st.markdown("""<div style="background:rgba(251,146,60,0.08);border:1px solid rgba(251,146,60,0.3);border-radius:12px;padding:20px;text-align:center;margin-top:16px;">
            <div style="color:#fb923c;font-weight:700;"> No scheduled classes found for this scope.</div>
    ...
    def col_label(slot):
        ...
        d = f" ({slot['date']})" if scope_type not in ["Particular Date","Particular Period"] else ""
        return f"{slot['subject']} [{slot['period']}]{t}{d}"

    col_labels = [col_label(s) for s in period_slots]

    # --- COORDINATION LOGIC: Filter students by Timetable Settings ---
    if "tt_settings" in st.session_state:
        tt_dept = st.session_state.tt_settings.get("department", "CSE(AI&ML)")
        tt_cls = st.session_state.tt_settings.get("class", "TY")
        if tt_dept.startswith("DEPARTMENT OF "):
    ...
    if not filtered_students:
        st.markdown(f"""<div style="background:rgba(244,114,182,0.08);border:1px solid rgba(244,114,182,0.3);border-radius:12px;padding:20px;text-align:center;margin-top:16px;">
            <div style="color:#f472b6;font-weight:700;"> No students found for Class {tt_cls} — {tt_dept}</div>
            <div style="color:#64748b;font-size:12px;margin-top:6px;">Go to Manage Students and add students to this exact Class and Branch to generate reports.</div>
        </div>""", unsafe_allow_html=True)
        return
    ...
    for student in filtered_students:
        ...
        for slot, lbl in zip(period_slots, col_labels):
            ...
            if erp_rec:
                ...
                if raw in ["Present", "Auto-Carried", "Exited", "Exited (Unverified)"]:
                    status = "Present"
                elif raw == "Present (Excused)":
                    status = "Excused"
                elif raw == "Late":
                    status = "Late"
                else:
            ...
            if status in ["Present","Late","Excused"]: total_present += 1
            if status == "Present":
                lbl_disp = " Present"
            elif status == "Late":
                lbl_disp = "⏱ Late"
        ...
        base["Att %"] = f"{pct}%"; base["Status"] = "OK " if pct>=75 else "Defaulter "
    ...
    with k3: st.metric(" Defaulters", defaulters)
    ...
    def _build_report_table(df_src, cols, title, accent):
        ...
        for i, (_, r) in enumerate(df_src.iterrows()):
            ...
            for c in _period_lbls:
                ...
                elif "E" in str(v) or "Excused" in str(v):
                    badge = f'<span style="background:rgba(56,189,248,0.12);color:#38bdf8;border:1px solid rgba(56,189,248,0.25);border-radius:5px;padding:2px 8px;font-size:10px;font-weight:700;"> E</span>'
                else:
    ...
    if not df_def_disp.empty:
        st.markdown(_build_report_table(df_def_disp, df_def_disp.columns, " Defaulters — Below 75%", "#f472b6"), unsafe_allow_html=True)
    else:
...
def show_ai_live_attendance_page():
    """Display the AI Live Attendance control panel in a premium split-screen layout."""
    ...
    st.markdown("""
        <div style="margin-bottom:24px;">
            ...
            <p style="color:#64748b; font-size:13px; margin:4px 0 0 0;">Real-time face recognition &amp; automated attendance logging</p>
        </div>
    """, unsafe_allow_html=True)
    
    # Init engine in session state if needed
    if 'recognition_engine' not in st.session_state:
        st.session_state.recognition_engine = None
    if 'recognition_running' not in st.session_state:
        st.session_state.recognition_running = False
    ...
    engine = st.session_state.recognition_engine
    is_running = st.session_state.recognition_running
    
    # SPLIT SCREEN LAYOUT: Left (Camera) & Right (Controls + Live Info)
    ...
    with right_col:
        ...
        with col_start:
            if st.button(" Start Recognition", disabled=is_running, use_container_width=True, type="primary", key="btn_start_rec"):
                try:
                    if not is_running:
        ...
        with col_stop:
            if st.button("⏹ Stop Recognition", disabled=not is_running, use_container_width=True, key="btn_stop_rec"):
                if is_running and engine:
                    if hasattr(engine, 'cam_running'):
                        engine.cam_running = False
                    if hasattr(engine, 'cap') and engine.cap and engine.cap.isOpened():
                        engine.cap.release()
                    if hasattr(engine, 'latest_frame'):
                        engine.latest_frame = None
                    st.session_state.recognition_running = False
        ...
        # Capture Button
        capture_btn = st.button(" Capture Unknown Face", disabled=not is_running, use_container_width=True, key="btn_cap_unk")
        if is_running and engine and hasattr(engine, 'latest_frame') and capture_btn:
            frame = engine.latest_frame
            if frame is not None:
                import cv2
                save_dir = os.path.join("dataset", "unknown")
                os.makedirs(save_dir, exist_ok=True)
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                filepath = os.path.join(save_dir, f"unknown_{timestamp}.jpg")
                cv2.imwrite(filepath, frame)
                st.toast(f" Saved unknown face to {filepath}")
                
        detected_student_placeholder = st.empty()
        exit_confirmation_placeholder = st.empty()
        auto_carry_placeholder = st.empty()
        st.markdown("---")
        
        active_students_placeholder = st.empty()
        event_logs_placeholder = st.empty()

    # Dynamic Streaming Render Logic
    def render_dashboard_state(update_buttons=True):
        cur_engine = st.session_state.recognition_engine
        cur_is_running = st.session_state.recognition_running
        
        # SYNC: Reload period_attendance from data.json so Reports reflects AI detections live
        if cur_is_running:
            try:
                with open("data.json", "r") as _f:
                    _app_data = json.load(_f)
                st.session_state.period_attendance = _app_data.get("period_attendance", st.session_state.period_attendance)
            except Exception:
                pass
        ...
        if cur_is_running:
            ...
            camera_placeholder.markdown("""
                <div class="cam-offline-box">
                    ...
                    <div style="font-size:16px; font-weight:700; color:#94a3b8; margin-bottom:6px;">Camera Offline</div>
        ...
        total_today = 0
        fps_val = "0.0"
        if cur_engine and hasattr(cur_engine, 'attendance_engine') and cur_engine.attendance_engine:
            active_count = len(cur_engine.attendance_engine.active_students)
            fps_val = f"{cur_engine.fps:.1f}" if hasattr(cur_engine, 'fps') else "0.0"
            try:
                today_str = datetime.now().strftime("%Y-%m-%d")
                today_data = cur_engine.attendance_engine.erp_data.get(today_str, {})
                total_today = sum(len(rolls) for subj in today_data.values() for rolls in subj.values())
            except Exception:
                total_today = 0
        ...
        with status_placeholder.container():
            ...
            with sc1:
                st.metric("Camera Engine", "Online" if cur_is_running else "Offline")
                st.metric("Active Students", active_count)
            with sc2:
                st.metric("Current FPS", fps_val)
                st.metric("Attendance Logged", total_today)
                
        # 3. Update Detected Student Card
        if cur_is_running and cur_engine and hasattr(cur_engine, 'last_detected_student') and cur_engine.last_detected_student:
            time_since_detection = time.time() - getattr(cur_engine, 'last_detected_time', 0)
            if time_since_detection < 10.0:
                last_student = cur_engine.last_detected_student
                roll_no = last_student.get('roll_no')
                similarity = last_student.get('similarity', 0.0)
                
                student_info = get_student_by_roll_no(roll_no)
                if student_info:
                    with detected_student_placeholder.container():
                        st.markdown("""
                            <div class="compact-card" style="border-left: 5px solid #10b981; margin-top: 15px;">
                                <div style="display: flex; align-items: center; gap: 8px;">
                                    <span style="font-size: 18px;"></span>
                                    <strong style="color: #10b981; font-size: 15px;">Detected Student Live Info</strong>
                                </div>
                            </div>
                        """, unsafe_allow_html=True)
                        ...
                        with card_col1:
                            if student_info.get('photo') and isinstance(student_info['photo'], bytes):
                                st.image(student_info['photo'], use_container_width=True)
                            else:
                                st.image("https://www.w3schools.com/howto/img_avatar.png", use_container_width=True)
                        with card_col2:
                            st.markdown(f"**Name**: `{student_info.get('name')}`")
                            st.markdown(f"**Roll No**: `{roll_no}`")
                            attendance_pct = calculate_attendance_percentage(roll_no)
                            st.markdown(f"**Attendance**: `{attendance_pct}%` | **Match**: `{similarity * 100:.1f}%`")
                            
                            today_str = datetime.now().strftime("%Y-%m-%d")
                            period = cur_engine.attendance_engine.get_current_period() if (cur_engine.attendance_engine) else None
                            if period:
                                subj = period.get('subject', 'Unknown')
                                per_id = period.get('period', 'Unknown')
                                try:
                                    erp_record = cur_engine.attendance_engine.erp_data.get(today_str, {}).get(subj, {}).get(per_id, {}).get(roll_no)
                                    if erp_record:
                                        st.caption(f" **{erp_record.get('status')}** logged at {erp_record.get('entry_time')}")
                                except Exception:
                                    pass
                else:
                    detected_student_placeholder.empty()
            else:
                detected_student_placeholder.empty()
        else:
            detected_student_placeholder.empty()
            
        # 3.5 Check for Pending Exits
        if update_buttons:
            if cur_engine and hasattr(cur_engine, 'attendance_engine') and cur_engine.attendance_engine:
                pending = getattr(cur_engine.attendance_engine, 'pending_exits', {})
                if pending:
                    # Process the first pending exit request
                    r_no = list(pending.keys())[0]
                    student_info = pending[r_no]
                    with exit_confirmation_placeholder.container():
                        st.markdown(f"""
                            <div style="background:rgba(244,114,182,0.1); border:1px solid #f472b6; border-radius:12px; padding:16px; margin-bottom:16px; text-align:center;">
                                <div style="color:#f472b6; font-weight:800; font-size:16px; margin-bottom:8px;">🚪 EXIT CONFIRMATION</div>
                                <div style="color:#e2e8f0; font-size:14px; margin-bottom:12px;">Scan detected again. Do you want to checkout, <b style="color:#ffffff;">{student_info['name']}</b>?</div>
                            </div>
                        """, unsafe_allow_html=True)
                        ec1, ec2 = st.columns(2)
                        with ec1:
                            if st.button("✅ Confirm Exit", key=f"confirm_exit_{r_no}", use_container_width=True, type="primary"):
                                cur_engine.attendance_engine.confirm_exit(r_no)
                                st.rerun()
                        with ec2:
                            if st.button("❌ Cancel", key=f"cancel_exit_{r_no}", use_container_width=True):
                                cur_engine.attendance_engine.cancel_exit(r_no)
                                st.rerun()
                else:
                    exit_confirmation_placeholder.empty()
            else:
                exit_confirmation_placeholder.empty()
            
        # 3.6 Check for Pending Auto Carries
        if update_buttons:
            if cur_engine and hasattr(cur_engine, 'attendance_engine') and cur_engine.attendance_engine:
                pending_ac = getattr(cur_engine.attendance_engine, 'pending_auto_carries', {})
                if pending_ac:
                    with auto_carry_placeholder.container():
                        st.markdown('<p style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#34d399;margin-bottom:8px;"> Pending Auto-Carry Verifications</p>', unsafe_allow_html=True)
                        
                        # Display Table of Pending Students
                        table_html = "<div style='background:rgba(52,211,153,0.05);border:1px solid rgba(52,211,153,0.2);border-radius:14px;overflow:hidden;margin-bottom:16px;'>"
                        table_html += "<table style='width:100%;border-collapse:collapse;font-family:Inter,sans-serif;'>"
                        table_html += "<thead><tr style='background:rgba(52,211,153,0.1);'>"
                        table_html += "<th style='padding:8px 14px;font-size:10px;font-weight:700;text-transform:uppercase;color:#34d399;text-align:left;border-bottom:1px solid rgba(52,211,153,0.2);'>Roll No</th>"
                        table_html += "<th style='padding:8px 14px;font-size:10px;font-weight:700;text-transform:uppercase;color:#34d399;text-align:left;border-bottom:1px solid rgba(52,211,153,0.2);'>Name</th>"
                        table_html += "<th style='padding:8px 14px;font-size:10px;font-weight:700;text-transform:uppercase;color:#34d399;text-align:right;border-bottom:1px solid rgba(52,211,153,0.2);'>Target Subject</th>"
                        table_html += "</tr></thead><tbody>"
                        for i, (r_no, data) in enumerate(pending_ac.items()):
                            bg = "rgba(52,211,153,0.02)" if i % 2 == 0 else "transparent"
                            name = data.get("name", "Unknown")
                            new_subj = data.get("new_subject", "")
                            table_html += f"<tr style='background:{bg};border-bottom:1px solid rgba(52,211,153,0.05);'>"
                            table_html += f"<td style='padding:8px 14px;color:#94a3b8;font-size:12px;font-weight:600;'>{r_no}</td>"
                            table_html += f"<td style='padding:8px 14px;color:#e2e8f0;font-size:13px;font-weight:600;'>{name}</td>"
                            table_html += f"<td style='padding:8px 14px;color:#34d399;font-size:12px;font-weight:700;text-align:right;'>{new_subj}</td>"
                            table_html += "</tr>"
                        table_html += "</tbody></table></div>"
                        st.markdown(table_html, unsafe_allow_html=True)

                        # Process the first pending auto carry request
                        r_no = list(pending_ac.keys())[0]
                        student_info = pending_ac[r_no]
                        st.markdown(f"""
                            <div style="background:rgba(52,211,153,0.1); border:1px solid #34d399; border-radius:12px; padding:16px; margin-bottom:16px; text-align:center;">
                                <div style="color:#e2e8f0; font-size:14px;">Is <b style="color:#ffffff;">{student_info['name']} ({r_no})</b> present for <b style="color:#ffffff;">{student_info['new_subject']}</b>?</div>
                            </div>
                        """, unsafe_allow_html=True)
                        ac1, ac2 = st.columns(2)
                        with ac1:
                            if st.button("✅ Yes, Present", key=f"confirm_ac_{r_no}", use_container_width=True, type="primary"):
                                cur_engine.attendance_engine.confirm_auto_carry(r_no)
                                st.rerun()
                        with ac2:
                            if st.button("❌ No, Absent", key=f"cancel_ac_{r_no}", use_container_width=True):
                                cur_engine.attendance_engine.cancel_auto_carry(r_no)
                                st.rerun()
                else:
                    auto_carry_placeholder.empty()
            else:
                auto_carry_placeholder.empty()
            
        # 4. Update Active Students and logs
        if cur_engine and hasattr(cur_engine, 'attendance_engine') and cur_engine.attendance_engine:
            active_students_dict = cur_engine.attendance_engine.active_students
            with active_students_placeholder.container():
                st.markdown('<p style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#64748b;margin-bottom:8px;"> In-Class Active Students</p>', unsafe_allow_html=True)
                if active_students_dict:
                    table_html = "<div style='background:rgba(12,17,33,0.85);border:1px solid rgba(79,158,255,0.1);border-radius:14px;overflow:hidden;margin-bottom:20px;'>"
                    table_html += "<table style='width:100%;border-collapse:collapse;font-family:Inter,sans-serif;'>"
                    table_html += "<thead><tr style='background:rgba(6,9,18,0.5);'>"
                    table_html += "<th style='padding:10px 14px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:#64748b;text-align:left;border-bottom:2px solid rgba(255,255,255,0.06);'>Roll No</th>"
                    table_html += "<th style='padding:10px 14px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:#64748b;text-align:left;border-bottom:2px solid rgba(255,255,255,0.06);'>Name</th>"
                    table_html += "<th style='padding:10px 14px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:#64748b;text-align:center;border-bottom:2px solid rgba(255,255,255,0.06);'>Period</th>"
                    table_html += "<th style='padding:10px 14px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:#4f9eff;text-align:right;border-bottom:2px solid rgba(79,158,255,0.18);'>Entry Time</th>"
                    table_html += "</tr></thead><tbody>"
                    for i, (r_no, data) in enumerate(active_students_dict.items()):
                        bg = "rgba(255,255,255,0.015)" if i % 2 == 0 else "transparent"
                        e_time = data.get("entry_time")
                        e_time_str = e_time.strftime("%H:%M:%S") if isinstance(e_time, datetime) else str(e_time)
                        name = data.get("name", "Unknown")
                        per = data.get("current_period", "-")
                        subj = data.get("current_subject", "-")
                        table_html += f"<tr style='background:{bg};border-bottom:1px solid rgba(255,255,255,0.03);'>"
                        table_html += f"<td style='padding:10px 14px;color:#94a3b8;font-size:12px;font-weight:600;'>{r_no}</td>"
                        table_html += f"<td style='padding:10px 14px;color:#e2e8f0;font-size:13px;font-weight:600;'>{name}</td>"
                        table_html += f"<td style='padding:10px 14px;color:#64748b;font-size:12px;text-align:center;'>{subj} [{per}]</td>"
                        table_html += f"<td style='padding:10px 14px;color:#4f9eff;font-size:12px;font-weight:700;text-align:right;font-family:\"JetBrains Mono\",monospace;'>{e_time_str}</td>"
                        table_html += "</tr>"
                    table_html += "</tbody></table></div>"
                    st.markdown(table_html, unsafe_allow_html=True)
                else:
                    st.markdown("""
                        <div style="background:rgba(255,255,255,0.02);border:1px dashed rgba(255,255,255,0.1);border-radius:12px;padding:20px;text-align:center;margin-bottom:20px;">
                            <div style="color:#64748b;font-size:13px;font-weight:600;">No active students currently detected in class.</div>
                        </div>
                    """, unsafe_allow_html=True)
                    
            with event_logs_placeholder.container():
                st.markdown('<p style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#64748b;margin-bottom:8px;"> Live System Logs</p>', unsafe_allow_html=True)
                events = cur_engine.attendance_engine.event_logs
                if events:
                    for event in reversed(events[-6:]):
                        e_type = event.get('type', 'EVENT')
                        name = event.get('name', 'Unknown')
                        subj = event.get('subject', '')
                        per_id = event.get('period', '')
                        e_time = event.get('time', '')
                        period_info = f" &bull; {subj} [{per_id}]" if subj else ""
                        time_info = f" &bull; {e_time}" if e_time else ""
                        
                        if e_type == 'ENTRY':
                            st.markdown(f"""
                                <div style="background:linear-gradient(90deg, rgba(52,211,153,0.1) 0%, transparent 100%);border-left:3px solid #34d399;border-radius:6px;padding:8px 12px;margin-bottom:6px;">
                                    <span style="color:#34d399;font-weight:800;font-size:11px;letter-spacing:0.05em;margin-right:8px;">PRESENT</span>
                                    <span style="color:#ffffff;font-size:13px;font-weight:600;">{name}</span>
                                    <span style="color:#64748b;font-size:11px;margin-left:6px;">{period_info}{time_info}</span>
                                </div>
                            """, unsafe_allow_html=True)
                        elif e_type == 'LATE':
                            st.markdown(f"""
                                <div style="background:linear-gradient(90deg, rgba(251,146,60,0.1) 0%, transparent 100%);border-left:3px solid #fb923c;border-radius:6px;padding:8px 12px;margin-bottom:6px;">
                                    <span style="color:#fb923c;font-weight:800;font-size:11px;letter-spacing:0.05em;margin-right:8px;">LATE</span>
                                    <span style="color:#ffffff;font-size:13px;font-weight:600;">{name}</span>
                                    <span style="color:#64748b;font-size:11px;margin-left:6px;">{period_info}{time_info}</span>
                                </div>
                            """, unsafe_allow_html=True)
                        elif e_type == 'AUTO-CARRY':
                            st.markdown(f"""
                                <div style="background:linear-gradient(90deg, rgba(168,85,247,0.1) 0%, transparent 100%);border-left:3px solid #a855f7;border-radius:6px;padding:8px 12px;margin-bottom:6px;">
                                    <span style="color:#a855f7;font-weight:800;font-size:11px;letter-spacing:0.05em;margin-right:8px;">AUTO-CARRIED</span>
                                    <span style="color:#ffffff;font-size:13px;font-weight:600;">{name}</span>
                                    <span style="color:#64748b;font-size:11px;margin-left:6px;">{period_info}{time_info}</span>
                                </div>
                            """, unsafe_allow_html=True)
                            ...
                            st.markdown(f"""
                                <div style="background:linear-gradient(90deg, rgba(244,114,182,0.1) 0%, transparent 100%);border-left:3px solid #f472b6;border-radius:6px;padding:8px 12px;margin-bottom:6px;">
                                    <span style="color:#f472b6;font-weight:800;font-size:11px;letter-spacing:0.05em;margin-right:8px;">EXIT</span>
                                    <span style="color:#ffffff;font-size:13px;font-weight:600;">{name}</span>
                                    <span style="color:#64748b;font-size:11px;margin-left:6px;">{period_info}{time_info}</span>
                                </div>
                            """, unsafe_allow_html=True)
                        else:
                            st.markdown(f'<div style="color:#94a3b8;font-size:12px;padding:6px 12px;">[{e_type}] {name}{period_info}</div>', unsafe_allow_html=True)
                else:
                    st.markdown('<div style="color:#64748b;font-size:13px;padding:10px;">No attendance events yet. Start recognition during a scheduled period.</div>', unsafe_allow_html=True)
        else:
            active_students_placeholder.markdown("""
                <div class="compact-card" style="text-align:center; padding:20px;">
                    <div style="font-size:28px;"></div>
                    <div style="color:#64748b; font-size:13px; margin-top:6px;">Start recognition to track active students</div>
                </div>
            """, unsafe_allow_html=True)
            event_logs_placeholder.markdown("""
                <div class="compact-card" style="text-align:center; padding:20px;">
                    <div style="font-size:28px;"></div>
                    <div style="color:#64748b; font-size:13px; margin-top:6px;">No events recorded yet</div>
                </div>
            """, unsafe_allow_html=True)

    # Render Initial State
    render_dashboard_state(update_buttons=True)
    
    # Run high-frequency, flicker-free local streaming loop inside python thread to eradicate page reloading
    if is_running:
        cur_engine = st.session_state.recognition_engine
        # Cache the initial pending counts
        last_pending_exits = len(cur_engine.attendance_engine.pending_exits) if cur_engine and getattr(cur_engine, 'attendance_engine', None) else 0
        last_pending_ac = len(cur_engine.attendance_engine.pending_auto_carries) if cur_engine and getattr(cur_engine, 'attendance_engine', None) else 0
        
        while st.session_state.recognition_running:
            cur_engine = st.session_state.recognition_engine
            # Check for changes in pending queues
            current_pending_exits = len(cur_engine.attendance_engine.pending_exits) if cur_engine and getattr(cur_engine, 'attendance_engine', None) else 0
            current_pending_ac = len(cur_engine.attendance_engine.pending_auto_carries) if cur_engine and getattr(cur_engine, 'attendance_engine', None) else 0
            
            if current_pending_exits != last_pending_exits or current_pending_ac != last_pending_ac:
                st.rerun()  # Forces a full UI refresh to render the new buttons safely
                
            render_dashboard_state(update_buttons=False)
            time.sleep(0.04)  # High performance 25 FPS update rate without any global reruns!
        ...
        if submit:
            ...
            if not name or not roll_no:
                ...
                try:
                    ...
                    if result.get('status') in ['success', 'partial_success']:
                        st.success(" AI Embeddings updated successfully! The student will now be recognized.")
                        # Hot-reload embeddings if the engine is currently running,
                        # otherwise cleanly stop the old thread and reset.
                        existing_engine = st.session_state.get('recognition_engine')
                        if existing_engine is not None:
                            if st.session_state.get('recognition_running', False):
                                # Engine is live — hot-reload without stopping
                                try:
                                    existing_engine.reload_embeddings()
                                    st.info(" Embeddings reloaded into running engine. No restart needed.")
                                except Exception as _re:
                                    st.warning(f"Hot-reload failed ({_re}). Stopping engine for clean restart.")
                                    if hasattr(existing_engine, 'cam_running'):
                                        existing_engine.cam_running = False
                                    st.session_state.recognition_running = False
                                    st.session_state.recognition_engine = None
                            else:
                                # Engine exists but is stopped — just clear it so next Start
                                # picks up fresh embeddings
                                st.session_state.recognition_engine = None
                        else:
                            st.session_state.recognition_engine = None
                    else:
